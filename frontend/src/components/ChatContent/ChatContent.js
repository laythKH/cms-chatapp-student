...
const ChatContent = ({ handleSelected, setSelect }) => {
  ...
  const [searchInputAddFriendGroup, setSearchInputAddFriendGroup] =
    useState("");
  ...
  const handleSearchGroupMembers = async (query) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `http://127.0.0.1:5000/api/v1/auth?search=${searchInputAddFriendGroup}`,
        config
      );
      // console.log(data);
  ...
  const fetchAllMessages = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `http://127.0.0.1:5000/api/v1/message/${selectedChat._id}`,
        config
      );
    ...
    <>
      <div className={`chat-container`}>
        ...
        <div className='chat-content'>
          ...
          {isTyping ? (
            <Lottie style={{ width: "100px" }} animationData={typingLoading} />
          ) : (
            <></>
          )}
        </div>
        <div style={{ padding: "16px 20px" }}>
          <Form onKeyDown={sendMessage} style={{ bottom: "20px" }}>
            <InputGroup>
      ...
      {!selectedChat?.isGroupChat ? ( //! If Not Group
        <Modal show={showMoreInfo} onHide={handleCloseMoreInfo}>
          ...
          <Modal.Body>
            <h1>
              {t("Chat.email")}: {personInfo.email}
            </h1>
            <h1>
              {t("Chat.role")}: {personInfo.role}
            </h1>
          </Modal.Body>
          <Modal.Footer
            style={{
        ...
        <Modal show={showMoreInfo} onHide={handleCloseMoreInfo} size='lg'>
          ...
          <Modal.Body>
              ...
              {selectedChat.users?.map((singleUser) => (
                ...
                >
                  ...
                  <Image
                    ...
                    onClick={() => removeFromGroup(singleUser)}
                  />
                </div>
              ))}
            </div>
            ...
            <div>
              <h5>{t("Chat.addFriends")}</h5>
              <FloatingLabel controlId='floatingInput' label='Add Users'>
                <Form.Control
                  type='text'
                  value={searchInputAddFriendGroup}
              ...
              {searchResultAddFriend &&
                searchResultAddFriend.map((singleUserResult) => (
                  <div style={{ display: "flex", alignItems: "center" }}>
                    <Friend
                      key={
                        singleUserResult.name + singleUserResult.studentNumber
                      }
                    ...
                    <Button
                      ...
                      onClick={() => handleAddPerson(singleUserResult)}
                    >
                      <Image
                        src={plusSVG}
                        roundedCircle
