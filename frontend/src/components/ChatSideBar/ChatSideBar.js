...
const ChatSideBar = ({ handleSelected, setSelect }) => {
  ...
  const handleSearchGroupMembers = async (query) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `http://127.0.0.1:5000/api/v1/auth?search=${searchGroup}`,
        config
      );
      // console.log(data);
  ...
  const handleDeleteSelectUserGroup = (singleUser) => {
    setSelectedUsersGroup(
      selectUsersGroup.filter((e) => e._id !== singleUser._id)
    );
  };
  ...
  return (
    <>
      ...
      <div className={`sidebar display-none`}>
        ...
        <div className='friends-container-holder'>
          ...
          <div className='friends-container'>
            {listChats.map((singleChat) => (
              <SingleChatCard singleChat={singleChat} setSelect={setSelect} />
            ))}
          </div>
        </div>
      </div>
      ...
      <Modal show={showAddSingleUser} onHide={handleCloseAddPerson}>
        ...
        <div
          style={{ maxHeight: "400px", overflowY: "scroll", padding: "0 20px" }}
        >
          ...
          {!isLoading &&
            ...
            searchResult?.map((singleUserResult) => (
              <Friend
                ...
                handleSelected={() => {
                  handleSelected(singleUserResult._id);
      ...
      <Modal show={showCreateGroup} onHide={handleCloseCreateGroup}>
        ...
        <div
          style={{ padding: "10px 20px", display: "flex", flexWrap: "wrap" }}
        >
          {selectUsersGroup?.map((singleUser) => (
            ...
            >
              ...
              <Image
                ...
                onClick={() => handleDeleteSelectUserGroup(singleUser)}
              />
            </div>
          ))}
        </div>
        <div
          style={{ maxHeight: "350px", overflowY: "scroll", padding: "0 20px" }}
        >
          {searchResultGroup &&
            searchResultGroup.map((singleUserResult) => (
              <div style={{ display: "flex", alignItems: "center" }}>
                ...
                <Button
                  ...
                  onClick={() =>
                    handleAddPersonToSelectUsersGroup(singleUserResult)
                  }
                >
                  <Image
                    src={plusSVG}
