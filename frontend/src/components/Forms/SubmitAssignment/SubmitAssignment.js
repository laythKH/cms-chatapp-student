...
const SubmitAssignment = () => {
  ...
  const handleShowSubmitAssigment = async (singleAssignment) => {
    ...
    const { data } = await axios.get(
      `http://localhost:5000/api/v1/solution/${singleAssignment?.assignment?._id}/${user?._id}`
    );
  ...
  const handleSubmitAssignmentSolution = async () => {
    ...
    const dueDateForAssignment = new Date(
  ...
  const getAllTask = async () => {
    const { data } = await axios.get(
      `http://localhost:5000/api/v1/solution/${user._id}`
    );
    console.log(data);
  ...
  return (
    <>
      <Table striped='columns' className='styled-table'>
        <thead>
          <tr>
            ...
            <th>{t("Home.assignment.submitAssignment.courseName")}</th>
            <th>{t("Home.assignment.createAssignment.input1")}</th>
            <th>{t("Home.assignment.createAssignment.due")}</th>
            <th>{t("Home.assignment.submitAssignment.showAss")}</th>
            <th>{t("Home.assignment.submitAssignment.subAss")}</th>
          </tr>
        </thead>
        <tbody>
          {courseWithAssgnment &&
            courseWithAssgnment?.map((single, idx) => (
              <tr key={idx + single?.name}>
                ...
                <td className='align-center'>
                  <Button
                    ...
                    onClick={() => handleShowAssignment(single)}
                  >
                    show
                  </Button>
                </td>
                <td
                  ...
                  onClick={() => handleShowSubmitAssigment(single)}
                >
      ...
      >
        ...
        <Modal.Footer
          style={{ color: "#d32f2f", fontWeight: "bold", fontSize: "14px" }}
        >
      ...
      >
        ...
        <Modal.Footer>
          {isThereSolution ? (
            <p
              style={{ color: "#d32f2f", fontSize: "14px", fontWeight: "600" }}
            >
              You Have Already Submit The Assignment
            </p>
          ) : (
            <Button
              style={{
              ...
              onClick={handleSubmitAssignmentSolution}
            >
              Submit Assignment
            </Button>
          )}
